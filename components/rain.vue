...
export default {
  ...
  mounted() {
    function isMobile() {
      ...
      RainyDay.prototype.initialize = function (options) {
        ...
        let defaults = {
          ...
          reflectionScaledownFactor: 1,
          reflectionDropMappingWidth: 1024,
          reflectionDropMappingHeight: 1024,
      ...
      RainyDay.prototype.prepareReflections = function () {
        ...
        console.log('[Rain_Reflection_Controller] - Creating element')
      ...
      RainyDay.prototype.rain = function (presets, speed) {
        ...
        console.log('[Rain_Controller] Collected Options', this.options)
        if (this.reflection !== this.REFLECTION_NONE) {
          this.prepareReflections()
          console.log('[Rain_Controller] - Preparing Reflections')
        }
        console.log('[Rain_Controller] - Starting animation toolkit')
        this.animateDrops()
      ...
      RainyDay.prototype.prepareBackground = function () {
        console.log('[Rain_Controller] - Preparing first pass on background')
    ...
    if (localStorage.getItem('🌧️') !== null && localStorage.getItem('🌧️') === "false") {
      return
    }
      if (!isMobile()) {
        let rainyDay = new RainyDay({
          image: document.body, // Target p element with ID, RainyDay.js will use backgorund image to simulate rain effects.
        });
      } else {
        console.log(`[Rain_Controller] - Skipping rain to improve device performance. Vendor is ${navigator.vendor}`)
        if (document.getElementById('bio') !== null) {
          document.getElementById('bio').innerHTML = document.getElementById('bio').innerHTML + "<span style=\"color: #fc4b38\"> Rain effect is disabled on mobile<br> to improve performance</span>"
        }
      }
